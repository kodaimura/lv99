...
func QueryStatus(accountId int, db *sqlx.DB) ([]AnswerStatus, error) {
	...
	err := db.Select(&answers, baseQuery, accountId)
	return answers, err
}

func QuerySearch(accountId, questionId int, level int, isCorrect *bool, commentAccountId int, db *sqlx.DB) ([]AnswerSearch, error) {
	...
	baseQuery := `
		SELECT
			...
			(SELECT COUNT(*) FROM comment c WHERE c.answer_id = a.id AND c.deleted_at Is NULL) as comment_count,
	...
	err := db.Select(&answers, baseQuery, args...)
	return answers, err
}
